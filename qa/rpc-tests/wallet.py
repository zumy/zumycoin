...
# Copyright (c) 2017 Zumy Developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

from test_framework.test_framework import ZumyTestFramework
from test_framework.util import *

class WalletTest (ZumyTestFramework):
    ...
    def check_fee_amount(self, curr_balance, balance_with_fee, fee_per_byte, tx_size):
        ...
        if fee < target_fee:
            raise AssertionError("Fee of %s ZMY too low! (Should be %s ZMY)"%(str(fee), str(target_fee)))
        # allow the node's estimation to be at most 2 bytes off
        if fee > fee_per_byte * (tx_size + 2):
            raise AssertionError("Fee of %s ZMY too high! (Should be %s ZMY)"%(str(fee), str(target_fee)))
        return curr_balance
    ...
    def run_test (self):
        ...
        # Send 210 ZMY from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 1000 ZMY in block rewards plus fees, but
        ...
        # Send 100 ZMY with subtract fee from amount
        ...
        # Sendmany 100 ZMY with subtract fee from amount
