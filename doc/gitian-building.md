...
*Setup instructions for a gitian build of Zumy using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Zumy
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-zumy-host/README.md).
...
- Storage on Physical hard drive: Zumyally Allocated 
...
Clone the git repositories for zumy and gitian and then checkout the zumy version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the zumy repository
...
To build Zumy (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the zumy repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zumy/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/zumy-zmy/zumy
...
For example:
```bash
URL=https://github.com/crowning-/zumy.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit zumy=${COMMIT} --url zumy=${URL} ../zumy/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zumy=${COMMIT} --url zumy=${URL} ../zumy/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zumy=${COMMIT} --url zumy=${URL} ../zumy/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zumy-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zumy-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/zumy-build.assert
```
