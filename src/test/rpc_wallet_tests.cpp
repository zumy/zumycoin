...
BOOST_AUTO_TEST_CASE(rpc_wallet)
{
    ...
    UniValue r;
    CPubKey demoPubkey;
    CZumyAddress demoAddress;
    UniValue retValue;
    std::string strAccount = "walletDemoAccount";
    CZumyAddress setaccountDemoAddress;
    {
        LOCK(pwalletMain->cs_wallet);

        demoPubkey = pwalletMain->GenerateNewKey(0, false);
        demoAddress = CZumyAddress(CTxDestination(demoPubkey.GetID()));
        ...
        setaccountDemoAddress = CZumyAddress(CTxDestination(setaccountDemoPubkey.GetID()));
    }
    /*********************************
     *          setaccount
     *********************************/
    ...
    BOOST_CHECK(CZumyAddress(retValue.get_str()).Get() == demoAddress.Get());
    ...
    BOOST_CHECK(CZumyAddress(arr[0].get_str()).Get() == demoAddress.Get());
