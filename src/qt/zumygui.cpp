...
const std::string ZumyGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ZumyGUI::DEFAULT_WALLET = "~Default";

ZumyGUI::ZumyGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void ZumyGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Zumy address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and zumy: URIs)"));
    ...
    aboutAction = new QAction(QIcon(":/icons/" + theme + "/about"), tr("&About Zumy"), this);
    aboutAction->setStatusTip(tr("Show information about Zumy"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Zumy"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Zumy addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Zumy addresses"));
    ...
    openAction->setStatusTip(tr("Open a zumy: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Zumy help message to get a list with possible Zumy command-line options"));
...
void ZumyGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool ZumyGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ZumyGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ZumyGUI::setWalletActionsEnabled(bool enabled)
{
...
void ZumyGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Zumy client") + " " + networkStyle->getTitleAddText();
...
void ZumyGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void ZumyGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ZumyGUI::showInfo()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showRPCConsole();
}

void ZumyGUI::showConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showRPCConsole();
}

void ZumyGUI::showGraph()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showRPCConsole();
}

void ZumyGUI::showPeers()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showRPCConsole();
}

void ZumyGUI::showRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showRPCConsole();
}

void ZumyGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void ZumyGUI::showSNConfEditor()
{
    GUIUtil::openSNConfigfile();
}

void ZumyGUI::showBackups()
{
    GUIUtil::showBackups();
}

void ZumyGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void ZumyGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
void ZumyGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ZumyGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ZumyGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ZumyGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ZumyGUI::gotoMasternodePage()
{
    masternodeAction->setChecked(true);
    if (walletFrame) walletFrame->gotoMasternodePage();
}

void ZumyGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ZumyGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void ZumyGUI::setNumConnections(int count)
{
    QString icon;
    QString theme = GUIUtil::getThemeName();
    switch(count)
...
void ZumyGUI::updateHeadersSyncProgressLabel()
{
...
void ZumyGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void ZumyGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void ZumyGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Zumy"); // default title
...
void ZumyGUI::changeEvent(QEvent *e)
{
...
void ZumyGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ZumyGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ZumyGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(ZumyUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void ZumyGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ZumyGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ZumyGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ZumyGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ZumyGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void ZumyGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void ZumyGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ZumyGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ZumyGUI::detectShutdown()
{
...
void ZumyGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ZumyGUI::showModalOverlay()
{
    if (modalOverlay)
        modalOverlay->showHide(false, true);
}

static bool ThreadSafeMessageBox(ZumyGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ZumyGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void ZumyGUI::unsubscribeFromCoreSignals()
{
...
void ZumyGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<ZumyUnits::Unit> units = ZumyUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const ZumyUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(ZumyUnits::name(unit)));
    }
...
{
    menu = new QMenu();
    Q_FOREACH(ZumyUnits::Unit u, ZumyUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(ZumyUnits::name(u)), this);
...
{
    setPixmap(QIcon(":/icons/light/unit_" + ZumyUnits::id(newUnits)).pixmap(33,STATUSBAR_ICONSIZE));
}
