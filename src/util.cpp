...
const char * const ZUMY_CONF_FILENAME = "zumy.conf";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "zumy" is a composite category enabling all Zumy-related debug output
            if(ptrCategory->count(std::string("zumy"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Zumy
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Zumy
    // Mac: ~/Library/Application Support/Zumy
    // Unix: ~/.zumy
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Zumy";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Zumy";
#else
    // Unix
    return pathRet / ".zumy";
#endif
                    ...
                    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create zumy.conf if it does not exist
        FILE* configFile = fopen(GetConfigFile().string().c_str(), "a");
        if (configFile != NULL) {
            // Write zumy.conf file with random username and password.
            WriteConfigFile(configFile);
            // New zumy.conf file written, now read it.
        }
    }
    ...
    {
        // Don't overwrite existing settings so command line settings override zumy.conf
