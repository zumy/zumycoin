...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CZumyAddress(CScriptID(redeemScript)).ToString();
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable) const
{
    ...
    // Tally
    std::map<CZumyAddress, CompactTallyItem> mapTally;
    for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        const CWalletTx& wtx = (*it).second;
    ...
    vecTallyRet.clear();
    BOOST_FOREACH(const PAIRTYPE(CZumyAddress, CompactTallyItem)& item, mapTally) {
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstantSend))
                {
                    if (nCoinType == ONLY_NOT500IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 2000 ZMY.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT500IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 5000 ZMY.");
                    ...
                    if (fUseInstantSend) {
                        if (nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                            strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 ZMY."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                        } else {
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CZumyAddress(address).ToString(), strPurpose))
        return false;
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CZumyAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CZumyAddress(address).ToString());
...
void CWallet::LockCoin(COutPoint& output)
{
    ...
    if (it != mapWallet.end()) it->second.MarkDirty(); // recalculate all zumy for this tx
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CZumyAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CZumyAddress(dest).ToString(), key);
}
