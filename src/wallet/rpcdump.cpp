...
void ImportAddress(const CZumyAddress& address, const std::string& strLabel);
void ImportScript(const CScript& script, const std::string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CZumyAddress(CScriptID(script)), strLabel);
    }
}
...
UniValue importpubkey(const UniValue& params, bool fHelp)
{
    ...
    ImportAddress(CZumyAddress(pubKey.GetID()), strLabel);
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"zumyaddress\"   (string, required) The zumy address for the private key\n"
            "\nResult:\n"
...
for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CZumySecret(key).ToString(), strTime);
