...
UniValue masternode(const UniValue& params, bool fHelp)
{
        ...
        (strCommand != "start" && strCommand != "start-alias" && strCommand != "start-all" && strCommand != "start-missing" &&
         ...
         strCommand != "connect" && strCommand != "outputs" && strCommand != "status"))
            throw std::runtime_error(
                ...
                "  start        - Start local Hot Masternode configured in zumy.conf\n"
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("payee",         CZumyAddress(winner->pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.vin, mn)) {
            mnObj.push_back(Pair("payee", CZumyAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
                ...
                strMode != "protocol" && strMode != "payee" && strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Zumy address associated with a Masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        BOOST_FOREACH(CMasternode& mn, vMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               CZumyAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                ...
                CZumyAddress address(mn.pubKeyCollateralAddress.GetID());
    ...
    if (strCommand == "decode")
    {
        ...
        BOOST_FOREACH(CMasternodeBroadcast& mnb, vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CZumyAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CZumyAddress(mnb.pubKeyMasternode.GetID()).ToString()));
